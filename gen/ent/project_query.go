...
func (pq *ProjectQuery) QueryTransactions() *TransactionQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, false, project.TransactionsTable, project.TransactionsColumn),
		)
...
func (pq *ProjectQuery) WithTransactions(opts ...func(*TransactionQuery)) *ProjectQuery {
	query := (&TransactionClient{config: pq.config}).Query()
...
func (pq *ProjectQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Project, error) {
	...
	if query := pq.withTransactions; query != nil {
		if err := pq.loadTransactions(ctx, query, nodes,
			func(n *Project) { n.Edges.Transactions = []*Transaction{} },
			func(n *Project, e *Transaction) { n.Edges.Transactions = append(n.Edges.Transactions, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *ProjectQuery) loadTransactions(ctx context.Context, query *TransactionQuery, nodes []*Project, init func(*Project), assign func(*Project, *Transaction)) error {
	...
	query.Where(predicate.Transaction(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(project.TransactionsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.project_transactions
		if fk == nil {
			return fmt.Errorf(`foreign-key "project_transactions" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "project_transactions" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
